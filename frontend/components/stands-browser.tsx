...
export function StandsBrowser({ initialData, initialCategory, initialSearch }: { initialData: PaginatedResult<Stand>; initialCategory?: string; initialSearch?: string }) {
  ...
  const update = useCallback((updates: Record<string, string | null>) => {
    ...
    Object.entries(updates).forEach(([key, value]) => value && value !== "tous" ? params.set(key, value) : params.delete(key));
    startTransition(() => router.push(`/stands?${params.toString()}`));
