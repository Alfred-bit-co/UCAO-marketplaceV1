...
export default function ProfilPage() {
  ...
  if (!profile) return <PageShell><main className="container-ucao py-[84px] text-center"><p className="text-xl font-medium">Connectez-vous pour gérer votre profil.</p></main></PageShell>;
      ...
      <main>
        ...
        <section className="container-ucao grid max-w-4xl gap-5 py-[54px] pb-[84px] md:grid-cols-[0.75fr_1.25fr]">
          <aside className="panel h-fit p-6">
            <div className="mb-5 grid size-16 place-items-center rounded-full bg-ucao-navy text-2xl font-bold text-white">
            ...
            {message && <Link className="text-center text-sm font-medium text-ucao-red hover:underline" href="/">Retour à la page d&apos;accueil</Link>}
          </aside>
          ...
          <form id="profile-form" className="panel grid gap-5 p-6" onSubmit={handleSubmit}>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="grid gap-1.5 font-medium">Prénom<input className="input-field" name="first_name" defaultValue={splitFullName(profile.full_name).firstName} required minLength={2} /></label>
              <label className="grid gap-1.5 font-medium">Nom<input className="input-field" name="last_name" defaultValue={splitFullName(profile.full_name).lastName} required minLength={2} /></label>
            </div>
            <label className="grid gap-1.5 font-medium">Email<input className="input-field opacity-70" value={profile.email} readOnly aria-readonly="true" /></label>
            <label className="grid gap-1.5 font-medium">Numéro de téléphone
